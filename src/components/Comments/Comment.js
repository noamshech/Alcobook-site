...
export default function Comment({ id, body, author, createdAt, likes,authorRef }) {
        ...
        <div className="media-body u-shadow-v18 g-bg-secondary g-pa-30">
          ...
          <div className="d-flex justify-content-between">
            ...
            {(edit  && (token.user._id===authorRef))? (
              <div>
                <button className="btn" value="discard changes" onClick={discard} >duscard changes</button>

                <button className="btn-danger" value="save changes" onClick={deleteComment} >deelete comment</button>
                <textarea className="textArea" onChange={e => setCommentBody(e.target.value)} defaultValue={commentBody} />

                <button className="btn" value="save changes" onClick={saveComment} >save changes</button>
              </div>
            ) : (
              <div >
