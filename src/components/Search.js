...
export default function Search({ setPosts }) {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    const search = {};
    if (cocktail !== '') search.title = cocktail;
    if (author !== '') search.author = author;
    if (ingredient !== '') search.ingredient = ingredient;
    if (body !== '') search.body = body;
    const searchResult = await postData(
      `${serverURL}/api/cocktail/search`,
      null,
      { search },
    );
  ...
  return (
    <div className='container col-md-8 mb-5 col-md-offset-6 d-flex align-items-start'>
      ...
      >
        <div id='myDIV' style={{ margin: '0.375rem', width: '86%' }}>
          <input
            type='text'
          ...
          {showSearchFields && (
            <input
              onChange={(e) => setSuthor(e.target.value)}
              type='text'
              className='form-control'
              placeholder='author'
              style={{ marginBottom: '0.375rem' }}
        ...
        <button
          ...
          style={{ padding: '0.6rem', alignSelf: 'flex-start' }}
        >
      ...
      <button
        className='btn btn-primary'
        onClick={() => setShowSearchFields((state) => !state)}
